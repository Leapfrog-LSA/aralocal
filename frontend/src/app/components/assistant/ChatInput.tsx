...
import type { AraLegalDocument, AraLegalMessage } from "../shared/types";

export interface ChatInputHandle {
    addDoc: (doc: AraLegalDocument) => void;
}

interface Props {
    onSubmit: (message: AraLegalMessage) => void;
    onCancel: () => void;
    isLoading: boolean;
...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput(
    ...
    const [attachedDocs, setAttachedDocs] = useState<AraLegalDocument[]>([]);
    ...
    const handleAddDocFromProject = useCallback((doc: AraLegalDocument) => {
        setAttachedDocs((prev) => {
            if (prev.some((d) => d.id === doc.id)) return prev;
            return [...prev, doc];
        });
