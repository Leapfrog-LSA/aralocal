...
import { getProject, listProjects, listStandaloneDocuments } from "@/app/lib/araLegalApi";
import type { AraLegalDocument, AraLegalProject } from "./types";

const CACHE_TTL_MS = 30_000;

interface DirectoryCache {
    standaloneDocuments: AraLegalDocument[];
...
export function useDirectoryData(enabled: boolean) {
    ...
    const [standaloneDocuments, setStandaloneDocuments] = useState<AraLegalDocument[]>([]);
    const [projects, setProjects] = useState<AraLegalProject[]>([]);
