...
import type { AraLegalDocument, AraLegalProject, AraLegalWorkflow } from "../shared/types";
import {
    getProject,
    listProjects,
...
interface Props {
    ...
    onAdd: (
        ...
        projectId?: string,
        documentIds?: string[],
        columnsConfig?: AraLegalWorkflow["columns_config"],
    ) => void;
    projects?: AraLegalProject[];
    /** When provided, skip the project/directory picker and show only these docs */
    projectDocs?: AraLegalDocument[];
    projectName?: string;
...
export function AddNewTRModal({
    ...
    const [projectDocs, setProjectDocs] = useState<AraLegalDocument[]>([]);
    const [loadingDocs, setLoadingDocs] = useState(false);

    // Full directory (when underProject is false)
    const [standaloneDocs, setStandaloneDocs] = useState<AraLegalDocument[]>([]);
    const [directoryProjects, setDirectoryProjects] = useState<AraLegalProject[]>(
        [],
    );
    ...
    // Workflow templates
    const [workflows, setWorkflows] = useState<AraLegalWorkflow[]>([]);
    ...
    const directoryFolders = isProjectMode
        ...
        : underProject
          ? []
          : directoryProjects;
    const flatProjectDocs: AraLegalDocument[] =
